...
impl GameLogic for TetrisLogic {
    ...
    fn step(&self, state: &Self::State, input: Self::Input) -> Self::State {
        ...
        let prev_lines = state.lines_cleared();
...
mod tests {
    ...
    fn score_bonus_is_added_per_line_cleared() {
        ...
        let mut state = TetrisCore::new(0);
        state.set_available_pieces(vec![Piece::O]);
        ...
        for x in 0..BOARD_WIDTH {
            ...
            state.set_cell(x, 0, 1);
        }
        // Sanity: row is not yet full.
        assert_eq!(state.snapshot().board[0].iter().filter(|&&c| c != 0).count(), BOARD_WIDTH - 2);

        state.set_current_piece_for_test(Piece::O, Vec2i::new(4, 1), 0);

        let logic_no_bonus = TetrisLogic::new(0, vec![Piece::O]);
        let next_no_bonus = logic_no_bonus.step(&state, InputAction::HardDrop);
        assert_eq!(next_no_bonus.lines_cleared(), 1);
        assert_eq!(next_no_bonus.score(), 100, "expected base 1-line clear score");
        ...
        assert_eq!(next_bonus.lines_cleared(), 1);
        assert_eq!(
            next_bonus.score(),
            200,
        ...
        assert!(next_bonus.current_piece().is_some());
        assert_eq!(next_bonus.current_piece_pos(), Vec2i::new(4, BOARD_HEIGHT as i32));
    }
}
